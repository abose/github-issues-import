...
def import_issues(issues):
    print('importing issues : ', len(issues))
    state.current = state.GENERATING

    print('getting all milestones')
    ...
    # 	except Exception as e:
    # 		print('label import failed')

    result_issues = []
    num_issues_copied = 0
    for issue in new_issues:

        num_issues_copied = num_issues_copied + 1
        print('issues', num_issues_copied, '/', num_issues)
